...
TEST_CASE("damage calculation basic") {
    ...
    // Actually, we can just manually set effectiveness if needed, or rely on loader if paths work.
    // For unit tests, it's better to be self-contained or mock.
    // Let's manually inject some data for the test.
    GameData::getInstance().addSpecies("TestMon", {"TestMon", 100, 100, 100, 100, 100, PokeType::Normal, PokeType::None});
    GameData::getInstance().setTypeEffectiveness(PokeType::Normal, PokeType::Normal, 1.0f);
    GameData::getInstance().setTypeEffectiveness(PokeType::Fire, PokeType::Grass, 2.0f);
    ...
    SECTION("Super effective move") {
        ...
        // Since we can't easily modify species of existing pokemon, let's make a new one.
        GameData::getInstance().addSpecies("GrassMon", {"GrassMon", 100, 100, 100, 100, 100, PokeType::Grass, PokeType::None});
