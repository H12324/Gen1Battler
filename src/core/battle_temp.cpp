...
void Battle::log(const std::string &message) { log( message); }
...
void Battle::apply_move(Pokemon &attacker, Pokemon &defender, const Move &move,
  ...
  if (effect.type == MoveEffectType::Damage ||
    ...
    if (result.type_effectiveness == 0.0f) {
      log( "It doesn't affect " << defender.name() << "...\n");
      return);
    }

    defender.take_damage(result.damage));
    log( defender.name() << " took " << result.damage << " damage!\n");
    ...
    if (eff_result.damage > 0) {
      ...
      log( defender.name() << " took " << eff_result.damage
                << " damage!\n");
    ...
    if (eff_result.recoil_damage > 0) {
      ...
      log( attacker.name() << " is hit with recoil!\n");
      if (attacker.hp() <= 0) {
        log( attacker.name() << " fainted from recoil!\n");
      }
    }
    ...
    if (eff_result.drain_amount > 0) {
      ...
      log( attacker.name() << " drained HP!\n");
    }

    if (!eff_result.message.empty()) {
      log( eff_result.message << "\n");
    }
  }
...
void Battle::switch_pokemon(int team_num, int new_index) {
  if (team_num == 1) {
    ...
    log( "Opponent sent out " << active2.name() << "!\n");
  }
}
