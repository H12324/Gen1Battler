...
DamageResult calculate_damage(const Pokemon& attacker, const Pokemon& defender, const Move& move) {
    ...
    if (!move.data) return result;
    if (move.data->category == MoveCategory::Status) return result;
    ...
    if (def == 0) def = 1; // Prevent divide by zero (Gen 1 softlock prevention)

    // 4. Base Damage Calculation
    // ((2 * Level * Crit / 5 + 2) * Power * A / D) / 50 + 2
    int damage = ((2 * level * crit_factor / 5 + 2) * move.data->power * atk / def) / 50 + 2;

    // 5. STAB
    if (move.data->type == attacker.type1() || move.data->type == attacker.type2()) {
        damage += damage / 2;
    }

    // 6. Type Effectiveness
    float type1_eff = GameData::getInstance().getEffectiveness(move.data->type, defender.type1());
    float type2_eff = 1.0f;
    if (defender.type2() != PokeType::None) {
        type2_eff = GameData::getInstance().getEffectiveness(move.data->type, defender.type2());
    }
